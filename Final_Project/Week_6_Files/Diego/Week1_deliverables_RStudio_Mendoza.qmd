...
# Check column names
print(names(global_energy_data))
print(names(cpi_data))

# Adjust column names based on actual names from the output above
# Example: if the column is named "Date" in both, rename to DATE
global_energy_data <- global_energy_data %>% rename(DATE = DATE)  # Replace DateColumn1 with actual column name
cpi_data <- cpi_data %>% rename(DATE = DATE)  # Replace DateColumn2 with actual column name

# Merge datasets on the DATE column
merged_data2 <- merge(global_energy_data, cpi_data, by = "DATE")

# Filter out rows with NA in the DATE column
merged_data2 <- merged_data2 %>% filter(!is.na(DATE))

# Line graph for Global Price of Energy Index and Renewable Energy Adoption Rates
ggplot(data = merged_data2, aes(x = DATE)) +
  geom_line(aes(y = EnergyIndex, color = "Energy Price Index"), size = 1) +
  geom_line(aes(y = RenewableAdoptionRate, color = "Renewable Adoption Rate"), size = 1) +
  labs(
    title = "Global Energy Price Index and Renewable Energy Adoption Over Time",
    x = "Date",
    y = "Value",
    color = "Legend"
  ) +
  theme_minimal() +
  scale_color_manual(values = c("Energy Price Index" = "blue", "Renewable Adoption Rate" = "green"))
...
# Adjust column names based on actual names from the output above
# Example: if the column is named "Date" in both, rename to DATE
global_energy_data <- global_energy_data %>% rename(DATE = DATE) # Adjust as necessary
cpi_data <- cpi_data %>% rename(DATE = DATE)  # Adjust as necessary
...
# Line graph for Global Price of Energy Index and Renewable Energy Adoption Rates
ggplot(data = merged_data2, aes(x = DATE)) +
  geom_line(aes(y = DATE, color = "Energy Price Index"), size = 1) +
  geom_line(aes(y = DATE, color = "Renewable Adoption Rate"), size = 1) +
  labs(
    title = "Global Energy Price Index and Renewable Energy Adoption Over Time",
    x = "Date",
    y = "Value",
...
print("Global energy data columns:")
print(names(global_energy_data))
print("CPI data columns:")
print(names(cpi_data))

# Rename or ensure both datasets have a common 'DATE' column for merging
global_energy_data <- global_energy_data %>% rename(DATE = DATE)  # Replace ActualDateColumn1 with correct name
cpi_data <- cpi_data %>% rename(DATE = DATE)  # Replace ActualDateColumn2 with correct name

# Merge datasets on the DATE column
merged_data2 <- merge(global_energy_data, cpi_data, by = "DATE")

# Check column names in the merged data to identify correct variable names
...
# Plotting the line graph with actual column names for energy price index and adoption rate
ggplot(data = merged_data2, aes(x = DATE)) +
  geom_line(aes(y = DATE, color = "Energy Price Index"), size = 1) +  # Replace CorrectColumnName1
  geom_line(aes(y = DATE, color = "Renewable Adoption Rate"), size = 1) +  # Replace CorrectColumnName2
  labs(
    title = "Global Energy Price Index and Renewable Energy Adoption Over Time",
    x = "Date",
    y = "Value",
